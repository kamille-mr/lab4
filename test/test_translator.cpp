...
TEST(Translator, current_calculating){

	Translator t("(2+7)*9-60/2");
	ASSERT_NO_THROW(t.Calculate(), 51);
}

TEST(Translator, current_calculating_negative){

	Translator t("30*2-70+4*(12/6)");
	ASSERT_NO_THROW(t.Calculate(), -2);
}

TEST(Translator, current_postfix_form){

	Translator t("6+2*1");
	ASSERT_EQ("621*+", t.GetPostfix());
}

TEST(Translator, current_infix_form){
...
TEST(Translator, can_parse_digits){
